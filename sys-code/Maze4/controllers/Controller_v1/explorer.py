...
class MazeExplorer:
    ...
    def _perceive(self):
        ...
        if self.step_i % C.CAMERA_EVERY == 0:
            ...
            # AND rgb_img already read above; feeds into a SEPARATE camera-only
            # obstacle map (NOT the lidar log-odds map) so the lidar can't
            # erase these cells -- see occupancy_grid.py
            # integrate_camera_obstacle(). Passing rgb_img lets it drop any
            # candidate that is actually the tracked blue/yellow object's own
            # coloured surface (see depth_obstacle.py's "SAFETY CHECK").
            obstacle_ranges, obstacle_bearings = self.depth_obstacle_detector.detect(
                depth_img, rgb_img)
