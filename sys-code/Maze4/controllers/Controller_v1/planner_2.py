...
class PathPlanner:
    ...
    def choose_target(self, grid, clusters, robot_xy, blocked, unknown):
        ...
        if best_path is not None:
            best_path = PathPlanner.smooth_path(best_path, blocked)
        return best_path, best

    # ---------------------------------------------------------------------- #
    @staticmethod
    def smooth_path(path_rc, blocked):
        """Smooth a grid path by replacing sequences with direct shortcuts.

        WHAT DOES THIS DO?
        -------------------
        A* produces a path one grid-cell at a time, resulting in many short
        steps with hard 90° turns at corners.  This function repeatedly scans
        the path and replaces runs of waypoints with a direct line wherever
        the straight connection stays clear of blocked cells.

        WHY DOES THIS HELP WITH CORNERING?
        ------------------------------------
        Before smoothing, a right-angle corner looks like:
          → → → ↓        (many waypoints at the corner cell itself)

        After smoothing, the last shortcut endpoint lands BEFORE the corner
        and the next one starts AFTER it:
          -----→            (diagonal short-cut bypasses the inner corner cell)
              ↘
               ↓

        Pure pursuit then steers toward a point that is already around the
        corner, so the robot sweeps a wide natural arc instead of cutting
        straight toward the inner wall.

        HOW IT WORKS:
          For each point A in the path, find the furthest point B such that
          the straight line A→B doesn't cross any blocked cell.
          Keep B and discard all waypoints between A and B.
          Repeat from B.

        The line-of-sight check uses Bresenham integer stepping so no cell
        on the straight line is skipped.

        Args:
            path_rc : list of (row, col) tuples from A*.
            blocked : inflated obstacle bool mask.

        Returns:
            Shorter, smoother list of (row, col) tuples.
        """
        if len(path_rc) <= 2:
            return path_rc
        ...
        while i < len(path_rc) - 1:
            # Try to skip as many intermediate points as possible from i.
            j = len(path_rc) - 1
            while j > i + 1:
                if PathPlanner._los_clear(blocked, path_rc[i], path_rc[j]):
                    break
                j -= 1
            smoothed.append(path_rc[j])
            i = j
        return smoothed

    @staticmethod
    def _los_clear(blocked, rc_a, rc_b):
        """True if the straight line from rc_a to rc_b avoids all blocked cells.

        Samples the line at every integer step using Bresenham-style rounding
        so no cell is accidentally skipped.

        Args:
            blocked : bool mask of inflated obstacles.
            rc_a    : (row, col) start cell.
            rc_b    : (row, col) end cell.

        Returns:
            True if the entire line is clear; False if any cell is blocked.
        """
        r0, c0 = rc_a
        r1, c1 = rc_b
        n = max(abs(r1 - r0), abs(c1 - c0))
        if n == 0:
            return not blocked[r0, c0]
        nrows, ncols = blocked.shape
        for k in range(n + 1):
            r = int(round(r0 + k * (r1 - r0) / n))
            c = int(round(c0 + k * (c1 - c0) / n))
            if 0 <= r < nrows and 0 <= c < ncols and blocked[r, c]:
                return False
        return True
