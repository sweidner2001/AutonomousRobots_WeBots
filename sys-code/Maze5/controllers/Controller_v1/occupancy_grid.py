...
The grid is indexed [row, col] = [y, x].  Row 0 / col 0 is the (GRID_ORIGIN_X,
GRID_ORIGIN_Y) corner.  Use world_to_grid / grid_to_world to convert.
"""
...
class OccupancyGrid:
    ...
    def integrate_scan(self, x, y, theta, ranges, bearings):
        ...
        for i in range(len(ranges)):
            rng = ranges[i]
            hit = True
            if not np.isfinite(rng) or rng >= C.LIDAR_MAX_RANGE * 0.999:
                # No return -> free space up to the capped range, no wall.
                rng = max_use
                hit = False
